...
const RemoveObject = () => {
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700  '>
      ...
      {/* <input onChange={(e) => setInput(e.target.value)} type="file" accept='image/*' className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-grey-600' required /> */}
        <input 
  onChange={(e) => setInput(e.target.files[0])} 
  type="file" 
  accept="image/*" 
  className="w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300 text-grey-600" 
  required 
/>
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#FF4938] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
         {
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 max-h-[600px]'>
        <div className='flex items-center gap-3'>
          <Scissors className='w-5 h-5 text-[#5A7AFF]' />
